...
const ProductListingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setDisplayedProducts(selectedCategory === 'All'
        ? products
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {loading ? (
          ...
          displayedProducts.map(product => (
            <Link to={`/products/${product.id}`} key={product.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 group">
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <span className="text-xl font-bold text-primary">₹{product.pricePerLitre} / Litre</span>
                  {getAvailabilityBadge(product.availability)}
                </div>
                <Link to={`/subscribe/${product.id}`}><Button variant="primary" className="w-full">Subscribe Now</Button></Link>
              </div>
            </Link>
          ))
        )}
