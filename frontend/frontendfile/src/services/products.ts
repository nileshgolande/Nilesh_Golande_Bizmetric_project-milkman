...
const mapApiProductToProduct = (apiProduct: ApiProduct): Product => ({
  ...
  category: typeof apiProduct.category === 'string' ? apiProduct.category : `Category ${apiProduct.category}`,
  pricePerLitre: Number(apiProduct.price),
  image: apiProduct.image_url || '/images/cow-milk.png',
  description: apiProduct.description || 'Fresh dairy product.',
...
export const fetchPublicProducts = async (): Promise<Product[]> => {
  const response = await fetch(`${API_BASE_URL}/api/product/public/`);
  if (!response.ok) {
